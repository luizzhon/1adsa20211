...
INSERT INTO Funcionario VALUES 
(null, 'Cecília Ribeiro', 2800, 'f', null, '1980-04-05', 105);
...
-- FUNCIONOUUUU ;)
INSERT INTO Funcionario VALUES 
(9, 'Alice Sousa', 2800, 'f', 4, '1980-04-05', 104);
...
SELECT salario, dataNasc FROM Funcionario where nomeFunc = 'João Silva';
...
SELECT nomeFunc, salario FROM Funcionario WHERE nomeFunc like '_%a';

SELECT nomeFunc FROM Funcionario WHERE nomeFunc like '%_n%';
...
SELECT idFunc as idFuncionario, nomeFunc, fkSupervisor as idSupervisor FROM Funcionario; 

SELECT idProj, fkDepto, fkGerente, nomeFunc, dataNasc FROM Projeto INNER JOIN
	Departamento on fkDepto = idDepto INNER JOIN Funcionario
		on fkGerente = idFunc;

SELECT idFunc, nomeFunc, fkProj, horas, nomeProj FROM Funcionario INNER JOIN
	FuncProj on fkFunc = idFunc inner join
		Projeto on fkProj = idProj;
...
SELECT TRUNCATE(avg(salario),2), sum(salario) FROM Funcionario;

SELECT min(salario), max(salario) FROM Funcionario;

SELECT idDepto, truncate(avg(salario),2), sum(salario) FROM Departamento INNER JOIN
	Funcionario on idDepto = fkDepto group by(idDepto);
    
SELECT idDepto, min(salario), max(salario) FROM Departamento INNER JOIN
	Funcionario on idDepto = fkDepto group by(idDepto);
...
SELECT Funcionario.idFunc as idFuncionario, Funcionario.nomeFunc as NomeFuncionario, Supervisor.idFunc as idSupervisor,
  Supervisor.nomeFunc as NomeSupervisor FROM Funcionario as Funcionario 
  INNER JOIN Funcionario as Supervisor
  ON funcionario.fkSupervisor = Supervisor.idFunc;

SELECT projeto.localProj, projeto.idProj, projeto.nomeProj, projeto.fkDepto,Funcionario.nomeFunc as 'Gerente', Funcionario.dataNasc FROM Projeto
     JOIN Departamento
     ON projeto.fkDepto = Departamento.idDepto
     JOIN Funcionario
     ON Departamento.fkGerente = Funcionario.idFunc;

SELECT * FROM Funcionario JOIN FuncProj ON fkFunc = idFunc JOIN Projeto ON idProj = fkProj;
